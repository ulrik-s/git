...
Generating the optional HTML summary depends on `gcov2perl`, which is
distributed with the Perl `Devel::Cover` package.  Install it with your
preferred tool (for example `brew install cpanminus && cpanm
Devel::Cover` or `cpan Devel::Cover`), or tell the build system where
to find the binary:
