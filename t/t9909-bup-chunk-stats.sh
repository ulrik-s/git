...
# New test for Git chunked storage

test_description='bup chunk storage statistics'

. ./test-lib.sh

TEST_REPO=chunk-stats

# Setup repository with initial 100k file

test_expect_success 'initialize repo with 100k file' '
...
# Loop 100 times, modify random 10-byte block in the file

test_expect_success 'perform 100 random edits and record stats' '
    (
        cd "$TEST_REPO" &&
        git config bup.chunking true &&
        : >../stats &&
